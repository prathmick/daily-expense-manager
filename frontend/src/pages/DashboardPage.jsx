...
import { getDashboardData, listBudgets } from "../db/services";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([getDashboardData(), listBudgets()])
      .then(([d, b]) => { setData(d); setBudgets(b); })
  ...
  return (
    <div className="page-content">
      {/* Hero */}
      <div style={{ background:"linear-gradient(135deg,#4f46e5,#7c3aed)", borderRadius:16, padding:"18px 16px", marginBottom:14, color:"#fff", position:"relative", overflow:"hidden" }}>
        ...
        <p style={{ margin:"6px 0 0", fontSize:11, opacity:0.65 }}>Today {fmt(data.today_total)} · Week {fmt(data.week_total)}</p>
      </div>
              ...
              <div key={item.category} style={{ marginBottom:10 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:5 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <span style={{ fontSize:16 }}>{ICONS[item.category]||"📦"}</span>
        ...
        {data.daily_totals_30d.every(d => !d.total) ? (
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={data.daily_totals_30d} margin={{ top:4, right:4, left:0, bottom:0 }}>
              ...
              <Tooltip formatter={v=>[fmt(v),"Spent"]} labelFormatter={fmtDate} contentStyle={{ borderRadius:10, border:"none", fontSize:12 }} />
            ...
            return (
              <div key={b.id} style={{ marginBottom:14 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <span style={{ fontSize:16 }}>{ICONS[b.category]||"📦"}</span>
                ...
                {pct>=80 && <p style={{ margin:"4px 0 0", fontSize:11, color, fontWeight:600 }}>{pct>=100?"⚠️ Over budget!":"⚡ Near limit"}</p>}
              </div>
            );
          })}
        </div>
